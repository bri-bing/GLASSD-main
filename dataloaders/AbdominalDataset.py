...
from dataloaders.niftiio import read_nii_bysitk                             #Input:nii image Output:numpy array

class mean_std_norm(object):                                                #一个用于归一化的类
...
def get_normalize_op(fids, domain=False):                                   #返回一个用于归一化的类实例，若domain为false，则是对单张图片的归一化，若domain为true，则是对一个数据集的图片归一化
...
class AbdominalDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains: list,  idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, extern_norm_fn = None, location_scale=False):
        ...
        for _domain in self.domains: # load file names
            ...
            #self.img_pids[_domain] = ['1','2',...]

        self.scan_ids = self.__get_scanids(mode, idx_pct) # train val test split in terms of patient ids  内容：{domain_name:['1','2','3',...]}

        self.info_by_scan = None
        self.sample_list = self.__search_samples(self.scan_ids) # 字典，内容：sample_list[domain][scan_id][img_fid/lbs_fid]
        if self.is_train:
        ...
        self.normalize_op = extern_norm_fn([ itm['img_fid'] for _, itm in self.sample_list[self.domains[0]].items() ])          #根据img来生成一个归一化操作
    ...
    def __get_scanids(self, mode, idx_pct):                                         #split multiple dataset according to the domain name and the mode(phase)
        """
    ...
    def __search_samples(self, scan_ids):
        ...
        return out_list                 #Structure:out_list[which_domain][which_id][img_or_label]
