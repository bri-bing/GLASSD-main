...
def train_one_epoch_SBF(model: torch.nn.Module, criterion: torch.nn.Module, projector: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer, optimizer_prompt: torch.optim.Optimizer,
    ...
    for i, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        content_gla,style_gla = model.forward_first_layer(GLA_img,0.1)
        content_sbf,style_sbf = model.forward_first_layer(mixed_img,0.1)
        content_gla_p = projector(content_gla)
        style_gla_p = projector(style_gla)
        content_sbf_p = projector(content_sbf)
        style_sbf_p = projector(style_sbf)
        content_loss = F.l1_loss(content_gla_p, content_sbf_p, reduction='mean') + F.l1_loss(content_sbf_p, content_gla_p, reduction='mean')
        style_loss = F.l1_loss(style_gla_p, style_sbf_p, reduction='mean') + F.l1_loss(style_sbf_p, style_gla_p, reduction='mean')
        style_loss = -style_loss
        sum_loss = content_loss + style_loss
        sum_loss.backward()
...
def prediction_wrapper(model, tau, test_loader, epoch, label_name, mode = 'base', save_prediction = False):
    """
