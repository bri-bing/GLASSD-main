...
from models.unet_ccsdg import Projector,UNetCCSDG
import numpy as np
import random
...
def get_obj_from_str(string, reload=False):                         #import 模块，并且实例化模块中的一个类
...
def instantiate_from_config(config):
    ...
    return get_obj_from_str(config["target"])(**config.get("params", dict()))   #target为模块中的类，并用params中的参数实例化

class DataModuleFromConfig(torch.nn.Module):
    def __init__(self, batch_size, train=None, validation=None, test=None,      #train、validation、test也是字典类型参数
                 num_workers=None):
        super().__init__()
    ...
    def prepare_data(self):                                     #根据dataset_configs字典当中的key进行实例化
        for data_cfg in self.dataset_configs.values():
            instantiate_from_config(data_cfg)

    def setup(self):                    #创建了一个新的字典dataset，他的key=['train','validation','test']，他们对应的value为实例化后的Dataset类（作为Dataloader的参数传入）
        self.datasets = dict(
...
if __name__ == "__main__":
    ...
    if getattr(model_config.params, 'pretrain') :
        ...
        param_prompt = [{"params": list(projector.parameters()) + [model.channel_prompt], "lr_scale": 1}]
    ...
    opt_prompt = eval(optimizer_config['target'])(param_prompt, **opt_params)
    ...
    if optimizer_config.lr_scheduler =='lambda':
        ...
        scheduler_prompt = lr_scheduler.LambdaLR(opt_prompt, lr_lambda=lambda_rule)
    else:
        scheduler=None
    ...
    data.prepare_data()
    data.setup()
    print(len(data.datasets["train"]))              #调用了Dataset类当中的__len__方法
    ...
    for cur_epoch in range(max_epoch):
        if SBF_config.usage:
            cur_iter = train_one_epoch_SBF(model, criterion, projector,train_loader, opt,opt_prompt,torch.device('cuda'),cur_epoch,cur_iter, optimizer_config.max_iter, SBF_config, visdir)
        else:
